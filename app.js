...
/* --- ميزة: الواجهة الترحيبية (Main UI) --- */
function renderMain() {
...
function renderSub(catName) {
    ...
    if (subs.length === 0) {
        renderItems(catName, "");
        return; // زر الرجوع سيتم تحديثه داخل renderItems
    }
...
/* --- ميزة: عرض المنتجات (Items Rendering) --- */
function renderItems(catName, sub) {
    ...
    // تحديث ذكي لزر الرجوع: إذا كان هناك تصنيفات فرعية عد للـ Sub، وإلا للرئيسية
    const itemsInCategory = menuData.filter(i => (i[`cat_${currentLang}`] || i.cat_fr) === catName);
    const hasSubs = [...new Set(itemsInCategory.map(i => i.sub).filter(s => s && s !== '-'))].length > 0;
    
    updateBackButton(hasSubs ? () => renderSub(catName) : showCategories);
}
